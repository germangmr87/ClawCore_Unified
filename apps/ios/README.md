...
This is an **alpha** iOS app that connects to an ClawCore Gateway as a `role: node`.
...
3. On iOS: ClawCore → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project ClawCore.xcodeproj -scheme ClawCore -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/ClawCoreKit` contains the shared transport/types used by the iOS app.
