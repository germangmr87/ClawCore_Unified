...
struct IOSGatewayChatTransport: ClawCoreChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ClawCoreChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ClawCoreChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ClawCoreChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ClawCoreChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ClawCoreChatAttachmentPayload]) async throws -> ClawCoreChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [ClawCoreChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(ClawCoreChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(ClawCoreGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ClawCoreGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ClawCoreChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ClawCoreAgentEventPayload.self)
                        {
