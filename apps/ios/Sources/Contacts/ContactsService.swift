...
final class ContactsService: ContactsServicing {
    ...
    func search(params: ClawCoreContactsSearchParams) async throws -> ClawCoreContactsSearchPayload {
        ...
        return ClawCoreContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: ClawCoreContactsAddParams) async throws -> ClawCoreContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return ClawCoreContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return ClawCoreContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> ClawCoreContactPayload {
        ClawCoreContactPayload(
