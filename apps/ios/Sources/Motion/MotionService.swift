...
final class MotionService: MotionServicing {
    func activities(params: ClawCoreMotionActivityParams) async throws -> ClawCoreMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[ClawCoreMotionActivityEntry], Error>) in
        ...
        return ClawCoreMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: ClawCorePedometerParams) async throws -> ClawCorePedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<ClawCorePedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = ClawCorePedometerPayload(
