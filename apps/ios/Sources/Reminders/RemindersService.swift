...
final class RemindersService: RemindersServicing {
    func list(params: ClawCoreRemindersListParams) async throws -> ClawCoreRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[ClawCoreReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return ClawCoreReminderPayload(
        ...
        return ClawCoreRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: ClawCoreRemindersAddParams) async throws -> ClawCoreRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = ClawCoreReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return ClawCoreRemindersAddPayload(reminder: payload)
    }
