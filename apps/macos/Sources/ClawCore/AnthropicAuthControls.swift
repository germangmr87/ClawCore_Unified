...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: ClawCoreOAuthStore.AnthropicOAuthStatus = ClawCoreOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: ClawCoreOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = ClawCoreOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = ClawCoreOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try ClawCoreOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. ClawCore can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: ClawCoreOAuthStore.AnthropicOAuthStatus,
