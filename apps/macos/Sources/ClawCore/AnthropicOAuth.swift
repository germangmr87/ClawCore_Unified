...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (ClawCore token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: ClawCoreOAuthStore.AnthropicOAuthStatus = ClawCoreOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.clawcore", category: "anthropic-oauth")
...
enum ClawCoreOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let clawcoreOAuthDirEnv = "CLAWCORE_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "ClawCore OAuth token file not found"
            case .unreadableFile: "ClawCore OAuth token file not readable"
            case .invalidJSON: "ClawCore OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in ClawCore OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "ClawCore OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.clawcoreOAuthDirEnv]?
