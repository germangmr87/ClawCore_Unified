...
enum CommandResolver {
    private static let projectRootDefaultsKey = "clawcore.gatewayProjectRootPath"
    private static let helperName = "clawcore"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let clawcoreEntry = root.appendingPathComponent("clawcore.mjs").path
        if FileManager().isReadableFile(atPath: clawcoreEntry) { return clawcoreEntry }
        let binEntry = root.appendingPathComponent("bin/clawcore.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/clawcore")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let clawcorePaths = self.clawcoreManagedPaths(home: home)
        if !clawcorePaths.isEmpty {
            extras.insert(contentsOf: clawcorePaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + clawcorePaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func clawcoreManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".clawcore"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func clawcoreExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectClawCoreExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("clawcore.mjs").path,
            root.appendingPathComponent("bin/clawcore.js").path,
        ]
    ...
    static func hasAnyClawCoreInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.clawcoreExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func clawcoreNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let clawcorePath = self.projectClawCoreExecutable(projectRoot: root) {
                return [clawcorePath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "clawcore", subcommand] + extraArgs
            }
            if let clawcorePath = self.clawcoreExecutable(searchPaths: searchPaths) {
                return [clawcorePath, subcommand] + extraArgs
            }

            let missingEntry = """
            clawcore entrypoint missing (looked for dist/index.js or clawcore.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real clawcore CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/clawcore"
        ...
        \(cliSection)
        \(projectSection)
        if command -v clawcore >/dev/null 2>&1; then
          CLI="$(command -v clawcore)"
          clawcore \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/clawcore.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/clawcore.mjs"
            node "$PRJ/clawcore.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/clawcore.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/clawcore.js"
            node "$PRJ/bin/clawcore.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent clawcore"
          pnpm --silent clawcore \(quotedArgs);
        else
          echo "clawcore CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ClawCoreConfigFile.loadDict()
