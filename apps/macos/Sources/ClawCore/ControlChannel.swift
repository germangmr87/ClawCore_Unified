...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ClawCoreProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.clawcore", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ClawCoreKit.AnyCodable]()) {
                $0[$1.key] = ClawCoreKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: ClawCoreProtocol.AnyCodable?) -> [String: ClawCoreProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ClawCoreProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ClawCoreKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ClawCoreProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("clawcore.control.heartbeat")
    static let controlAgentEvent = Notification.Name("clawcore.control.agent")
}
