...
enum DebugActions {
    private static let verboseDefaultsKey = "clawcore.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "clawcore.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = ClawCorePaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "ClawCore", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = ClawCorePaths.configURL
        guard
