...
private let deepLinkLogger = Logger(subsystem: "ai.clawcore", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "ClawCore is paused", message: "Unpause ClawCore to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run ClawCore agent?", message: body) else { return }
        }
