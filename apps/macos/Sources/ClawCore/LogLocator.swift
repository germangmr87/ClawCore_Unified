...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["CLAWCORE_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/clawcore")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("clawcore-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("clawcore-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/clawcore/ (rolling or stdout), or nil if none exist.
