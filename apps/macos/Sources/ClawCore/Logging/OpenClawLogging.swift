...
enum ClawCoreLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = ClawCoreOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ClawCoreFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ClawCoreLogging.bootstrapIfNeeded()
        let label = ClawCoreLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ClawCoreOSLogHandler: LogHandler {
...
struct ClawCoreFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ClawCoreLogging.parseLabel(self.label)
