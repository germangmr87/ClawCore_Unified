...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.clawcore", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: ClawCorePeekabooBridgeServices?
    private static var clawcoreSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("ClawCore", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = ClawCorePeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.clawcoreSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.clawcoreSocketPath, privacy: .public)")
    }
...
@MainActor
private final class ClawCorePeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.clawcore.peekaboo")
