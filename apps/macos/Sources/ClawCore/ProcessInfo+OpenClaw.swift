...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.clawcore.mac`) even if the shipped
    ...
    {
        if environment["CLAWCORE_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "clawcore.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "clawcore.nixMode") { return true }
