...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.clawcore", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ClawCoreSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ClawCoreSessionPreviewEntry,
    ...
    private static func cache(payload: ClawCoreSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ClawCoreChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ClawCoreKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ClawCoreKit.AnyCodable]) -> [ClawCoreChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(ClawCoreChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: ClawCoreChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ClawCoreChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ClawCoreChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ClawCoreChatMessage) -> String? {
