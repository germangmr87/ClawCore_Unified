...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.clawcore/clawcore.json. Prefer CLAWCORE_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.clawcore/clawcore.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.clawcore/clawcore.json. Restart the gateway to apply."
        }
