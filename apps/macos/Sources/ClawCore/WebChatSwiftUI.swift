...
private let webChatSwiftLogger = Logger(subsystem: "ai.clawcore", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: ClawCoreChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ClawCoreChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(ClawCoreChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ClawCoreChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ClawCoreGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    ClawCoreAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ClawCoreChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ClawCoreChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ClawCoreChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: ClawCoreChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ClawCoreChatView>) -> NSViewController
    {
