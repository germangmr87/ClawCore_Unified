...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: ClawCoreProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ClawCoreProtocol.AnyCodable] = [
            "presence": .init([ClawCoreProtocol.AnyCodable(entry)]),
        ]
        let payload = ClawCoreProtocol.AnyCodable(payloadMap)
