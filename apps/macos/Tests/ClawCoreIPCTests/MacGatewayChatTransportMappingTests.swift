...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: ClawCoreProtocol.AnyCodable(["ok": ClawCoreProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: ClawCoreProtocol.AnyCodable(["ok": ClawCoreProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = ClawCoreProtocol.AnyCodable([
            "runId": ClawCoreProtocol.AnyCodable("run-1"),
            "sessionKey": ClawCoreProtocol.AnyCodable("main"),
            "state": ClawCoreProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: ClawCoreProtocol.AnyCodable(["a": ClawCoreProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
