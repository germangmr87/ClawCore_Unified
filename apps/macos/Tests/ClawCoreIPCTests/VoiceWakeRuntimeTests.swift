...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["clawcore"]
        let text = "hey clawcore"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey clawcore do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["clawcore"], minPostTriggerGap: 0.3)
