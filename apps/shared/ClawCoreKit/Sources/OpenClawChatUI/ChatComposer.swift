...
@MainActor
struct ClawCoreChatComposer: View {
    @Bindable var viewModel: ClawCoreChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ClawCoreChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ClawCoreChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ClawCorePendingAttachment.id)
                { (att: ClawCorePendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ClawCorePlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(ClawCoreChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ClawCoreChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ClawCoreChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ClawCoreChatTheme.subtleCard)
        .clipShape(Capsule())
    }
