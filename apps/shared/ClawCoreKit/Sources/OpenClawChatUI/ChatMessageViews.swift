...
struct ChatMessageBubble: View {
    let message: ClawCoreChatMessage
    let style: ClawCoreChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? ClawCoreChatTheme.userText : ClawCoreChatTheme.assistantText
    ...
    private var inlineAttachments: [ClawCoreChatMessageContent] {
    ...
    private var toolCalls: [ClawCoreChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [ClawCoreChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? ClawCoreChatTheme.userBubble
        }
        if self.style == .onboarding {
            return ClawCoreChatTheme.onboardingAssistantBubble
        }
        return ClawCoreChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return ClawCoreChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: ClawCoreChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? ClawCoreChatTheme.userText : ClawCoreChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: ClawCoreChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ClawCoreChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(ClawCoreChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [ClawCoreChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: ClawCoreChatTheme.assistantText)
            }
        }
    }
}
