...
@MainActor
public struct ClawCoreChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: ClawCoreChatViewModel
    ...
    public init(
        viewModel: ClawCoreChatViewModel,
    ...
    private var visibleMessages: [ClawCoreChatMessage] {
        let base: [ClawCoreChatMessage]
    ...
    private func mergeToolResults(in messages: [ClawCoreChatMessage]) -> [ClawCoreChatMessage] {
        ...
        for message in messages {
            ...
            let merged = ClawCoreChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: ClawCoreChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func toolCallIds(in message: ClawCoreChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: ClawCoreChatMessage) -> String {
