...
private let chatUILogger = Logger(subsystem: "ai.clawcore", category: "ClawCoreChatUI")

@MainActor
@Observable
public final class ClawCoreChatViewModel {
    public private(set) var messages: [ClawCoreChatMessage] = []
    ...
    public var attachments: [ClawCorePendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ClawCoreChatPendingToolCall] = []
    public private(set) var sessions: [ClawCoreChatSessionEntry] = []
    private let transport: any ClawCoreChatTransport
    ...
    private var pendingToolCallsById: [String: ClawCoreChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any ClawCoreChatTransport) {
    ...
    public var sessionChoices: [ClawCoreChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [ClawCoreChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ClawCorePendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ClawCoreChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ClawCoreChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [ClawCoreChatMessage]) -> [ClawCoreChatMessage] {
        var result: [ClawCoreChatMessage] = []
    ...
    private static func dedupeKey(for message: ClawCoreChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ClawCoreChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> ClawCoreChatAttachmentPayload in
            ClawCoreChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ClawCoreChatSessionEntry {
        ClawCoreChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: ClawCoreChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ClawCoreChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ClawCoreAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ClawCoreChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ClawCorePlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
