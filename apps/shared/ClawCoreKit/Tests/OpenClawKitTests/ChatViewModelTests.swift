...
import Foundation
import Testing
@testable import ClawCoreChatUI
...
private final class TestChatTransport: @unchecked Sendable, ClawCoreChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ClawCoreChatHistoryPayload]
    private let sessionsResponses: [ClawCoreChatSessionsListResponse]

    private let stream: AsyncStream<ClawCoreChatTransportEvent>
    private let continuation: AsyncStream<ClawCoreChatTransportEvent>.Continuation

    init(
        historyResponses: [ClawCoreChatHistoryPayload],
        sessionsResponses: [ClawCoreChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<ClawCoreChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ClawCoreChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ClawCoreChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [ClawCoreChatAttachmentPayload]) async throws -> ClawCoreChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return ClawCoreChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ClawCoreChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ClawCoreChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ClawCoreChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = ClawCoreChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = ClawCoreChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { ClawCoreChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = ClawCoreChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = ClawCoreChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = ClawCoreChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { ClawCoreChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
