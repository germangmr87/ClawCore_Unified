...
This document describes how the ClawCore ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`clawcore acp` exposes an ACP agent over stdio and forwards prompts to a running
ClawCore Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a ClawCore Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `clawcore acp` over stdio.

Example config:

```bash
clawcore config set gateway.remote.url wss://gateway-host:18789
clawcore config set gateway.remote.token <token>
```

Example run:

```bash
clawcore acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
clawcore acp --session agent:main:main
clawcore acp --session agent:design:main
clawcore acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “ClawCore ACP” to start a thread.

## Execution Model

- ACP client spawns `clawcore acp` and speaks ACP messages over stdio.
...
```bash
clawcore acp --session agent:main:main
clawcore acp --session-label "support inbox"
clawcore acp --reset-session
```
...
`clawcore acp` resolves the Gateway URL and auth from CLI flags or config:
