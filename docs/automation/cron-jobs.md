...
- Jobs persist under `~/.clawcore/cron/` so restarts don’t lose schedules.
...
clawcore cron list
clawcore cron run <job-id>
clawcore cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.clawcore/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `clawcore cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, ClawCore defaults to `announce`.
...
- Job store: `~/.clawcore/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.clawcore/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
clawcore cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
clawcore cron edit <jobId> --agent ops
clawcore cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
clawcore cron run <jobId>
clawcore cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
clawcore cron edit <jobId> \
...
```bash
clawcore system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`clawcore system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `CLAWCORE_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- ClawCore applies exponential retry backoff for recurring jobs after consecutive errors:
