...
clawcore system heartbeat last
clawcore logs --follow
clawcore config get agents.defaults.heartbeat
clawcore channels status --probe
```
...
```bash
clawcore config get agents.defaults.heartbeat.activeHours
clawcore config get agents.defaults.heartbeat.activeHours.timezone
clawcore config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
clawcore cron list
clawcore logs --follow
```
