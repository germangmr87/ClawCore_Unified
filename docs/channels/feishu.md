...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects ClawCore to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
clawcore plugins install @clawcore/feishu
```

Local checkout (when running from a git repo):

```bash
clawcore plugins install ./extensions/feishu
```
...
If you just installed ClawCore, run the wizard:
...
2. Configuring app credentials in ClawCore
...
1. You already ran `clawcore channels add` for Feishu
2. The gateway is running (`clawcore gateway status`)
...
```bash
clawcore pairing approve feishu <CODE>
```
...
2. Run `clawcore logs --follow` and look for `chat_id`
...
2. Run `clawcore logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `clawcore gateway status`  | Show gateway status           |
| `clawcore gateway install` | Install/start gateway service |
| `clawcore gateway stop`    | Stop gateway service          |
| `clawcore gateway restart` | Restart gateway service       |
| `clawcore logs --follow`   | Tail gateway logs             |
...
4. Check logs: `clawcore logs --follow`
...
5. Ensure the gateway is running: `clawcore gateway status`
6. Check logs: `clawcore logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.clawcore/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.clawcore/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
