...
Matrix is an open, decentralized messaging protocol. ClawCore connects as a Matrix **user**
...
```bash
clawcore plugins install @clawcore/matrix
```

Local checkout (when running from a git repo):

```bash
clawcore plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `clawcore plugins install @clawcore/matrix`
   - From a local checkout: `clawcore plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: ClawCore calls the same
     login endpoint, stores the access token in `~/.clawcore/credentials/matrix/credentials.json`,
...
- On first connection, ClawCore requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  ClawCore logs a warning.
...
`~/.clawcore/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `clawcore pairing list matrix`
  - `clawcore pairing approve matrix <CODE>`
...
- On startup, ClawCore resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
