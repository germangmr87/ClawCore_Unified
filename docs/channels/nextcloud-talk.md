...
```bash
clawcore plugins install @clawcore/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
clawcore plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "ClawCore" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `clawcore pairing list nextcloud-talk`
  - `clawcore pairing approve nextcloud-talk <CODE>`
