...
clawcore pairing approve telegram <CODE>
```
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `clawcore doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
...
```
    ...
    - or read `chat.id` from `clawcore logs --follow`
...
- DM messages can carry `message_thread_id`; ClawCore routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    ClawCore can stream partial replies by sending a temporary Telegram message and editing it as text arrives.
    ...
    For text-only replies, ClawCore keeps the same preview message and performs a final edit in place (no second message).

    For complex replies (for example media payloads), ClawCore falls back to normal final delivery and then cleans up the preview message.

    `streamMode` is separate from block streaming. When block streaming is explicitly enabled for Telegram, ClawCore skips the preview stream to avoid double-streaming.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, ClawCore retries as plain text.
...
```
    ...
    - `~/.clawcore/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, ClawCore enqueues system events like:
  ...
  <Accordion title="Ack reactions">
    `ackReaction` sends an acknowledgement emoji while ClawCore is processing an inbound message.
...
```bash
clawcore message send --channel telegram --target 123456789 --message "hi"
clawcore message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `clawcore channels status` warns when config expects unmentioned group messages.
    - `clawcore channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `clawcore logs --follow` for skip reasons
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `open` requires `"*"`. `clawcore doctor --fix` can resolve legacy `@username` entries to IDs.
- `channels.telegram.groupPolicy`: `open | allowlist | disabled` (default: allowlist).
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `clawcore doctor --fix` can resolve legacy `@username` entries to IDs.
