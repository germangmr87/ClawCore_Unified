...
Tlon is a decentralized messenger built on Urbit. ClawCore connects to your Urbit ship and can
...
```bash
clawcore plugins install @clawcore/tlon
```

Local checkout (when running from a git repo):

```bash
clawcore plugins install ./extensions/tlon
```
...
By default, ClawCore blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `clawcore message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, ClawCore replies in-thread.
