...
read_when:
  - Setting up Twitch chat integration for ClawCore
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. ClawCore connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
clawcore plugins install @clawcore/twitch
```

Local checkout (when running from a git repo):

```bash
clawcore plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `CLAWCORE_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "clawcore", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use CLAWCORE_TWITCH_ACCESS_TOKEN env var)
...
```bash
CLAWCORE_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
