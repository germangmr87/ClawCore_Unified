...
- `/usage cost` in chats: local cost summary aggregated from ClawCore session logs.
- CLI: `clawcore status --usage` prints a full per-provider breakdown.
- CLI: `clawcore channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
