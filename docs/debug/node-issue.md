...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ClawCore
...
Running ClawCore via Node with `tsx` fails at startup with:

```
[clawcore] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `clawcore status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch clawcore.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node clawcore.mjs status` works on Node 25.
