...
read_when:
  - Designing workspace memory (~/.clawcore/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep ClawCore integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.clawcore/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.clawcore/workspace` as canonical human-readable memory.
...
```
~/.clawcore/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in ClawCore**, but keep a separable core library.

### Why integrate into ClawCore?

- ClawCore already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `clawcore memory recall "…" --k 25 --since 30d`
  - `clawcore memory reflect --since 7d`
...
Pragmatic take for `~/.clawcore/workspace`:
