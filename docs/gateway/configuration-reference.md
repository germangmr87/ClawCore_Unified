...
description: "Complete field-by-field reference for ~/.clawcore/clawcore.json"
---

# Configuration Reference

Every field available in `~/.clawcore/clawcore.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — ClawCore uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `clawcore doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["clawcore-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `clawcore plugins install @clawcore/mattermost`.
...
ClawCore spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@clawcore"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `clawcore.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, ClawCore auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, ClawCore assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.clawcore/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.clawcore/agents/main/agent",
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:clawcore`   | All built-in tools (excludes provider plugins)                                           |
...
ClawCore uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.clawcore/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `CLAWCORE_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `clawcore onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `clawcore onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `clawcore onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `clawcore onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `clawcore onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `clawcore onboard --auth-choice minimax-api`.
...
- Loaded from `~/.clawcore/extensions`, `<workspace>/.clawcore/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or CLAWCORE_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `CLAWCORE_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.clawcore-dev` + port `19001`), `--profile <name>` (uses `~/.clawcore-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.clawcore/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-clawcore-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `CLAWCORE_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or CLAWCORE_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__clawcore__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__clawcore__/a2ui/`
...
- Also serves A2UI at `/__clawcore__/a2ui/`.
...
- Hostname defaults to `clawcore`. Override with `CLAWCORE_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.clawcore/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `clawcore dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.clawcore/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${CLAWCORE_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.clawcore/credentials/oauth.json`.
...
- Default log file: `/tmp/clawcore/clawcore-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `clawcore doctor --fix` can strip unknown keys).
