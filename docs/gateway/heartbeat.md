...
- During heartbeat runs, ClawCore treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `clawcore sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, ClawCore skips the heartbeat run entirely (no model call).
...
```bash
clawcore system event --text "Check for urgent follow-ups" --mode now
```
