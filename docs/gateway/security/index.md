...
## Quick check: `clawcore security audit`
...
```bash
clawcore security audit
clawcore security audit --deep
clawcore security audit --fix
```
...
- Tighten local perms (`~/.clawcore` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

ClawCore is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, ClawCore also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.clawcore/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.clawcore/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.clawcore/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.clawcore/credentials/oauth.json`
...
`clawcore security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${CLAWCORE_GATEWAY_PASSWORD}
```
...
ClawCore stores session transcripts on disk under `~/.clawcore/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.clawcore` (see the audit section below). If you need
...
ClawCore can refresh the skills list mid-session:
...
- If you install plugins from npm (`clawcore plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.clawcore/extensions/<pluginId>/` (or `$CLAWCORE_STATE_DIR/extensions/<pluginId>/`).
  - ClawCore uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
clawcore pairing approve <channel> <code>
```
...
By default, ClawCore routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
ClawCore has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.clawcore/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.clawcore or your logs.”
...
4. **Re-run audit**
   - `clawcore security audit --deep` and confirm the report is clean.
...
- `~/.clawcore/clawcore.json`: `600` (user read/write only)
- `~/.clawcore`: `700` (user only)

`clawcore doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `CLAWCORE_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__clawcore__/canvas/` and `/__clawcore__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_clawcore-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `CLAWCORE_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `clawcore doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `CLAWCORE_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `CLAWCORE_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), ClawCore
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. ClawCore verifies the identity by resolving the
...
- ClawCore will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.clawcore/` (or `$CLAWCORE_STATE_DIR/`) may contain secrets or private data:

- `clawcore.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `clawcore status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@clawcore", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.clawcore/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `clawcore` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only ClawCore clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default ClawCore limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `clawcore gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `CLAWCORE_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/clawcore/clawcore-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.clawcore/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + ClawCore version
...
Found a vulnerability in ClawCore? Please report responsibly:

1. Email: [security@clawcore.ai](mailto:security@clawcore.ai)
