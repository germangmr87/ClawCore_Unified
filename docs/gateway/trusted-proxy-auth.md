...
read_when:
  - Running ClawCore behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of ClawCore
...
- You run ClawCore behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. ClawCore checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. ClawCore extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://clawcore.example.com
    to: http://clawcore-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
clawcore.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy clawcore:18789 {
...
location / {
    ...
    proxy_pass http://clawcore:18789;
...
`clawcore security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update ClawCore config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `clawcore security audit` and review findings
