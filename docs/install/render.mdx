...
Deploy ClawCore on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/clawcore/clawcore)
...
repository configures everything needed to run ClawCore:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: CLAWCORE_STATE_DIR
        value: /data/.clawcore
      - key: CLAWCORE_WORKSPACE_DIR
        value: /data/workspace
      - key: CLAWCORE_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/clawcore`.
...
If you use the original ClawCore repository, Render will not auto-deploy your ClawCore. To update it, run a manual Blueprint sync from the dashboard.
...
For ClawCore, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any ClawCore host.
