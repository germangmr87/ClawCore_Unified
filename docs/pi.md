...
This document describes how ClawCore integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

ClawCore uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, ClawCore directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in ClawCore's local TUI mode)                                             |
...
├── pi-tools.ts                    # createClawCoreCodingTools()
...
├── clawcore-tools.ts              # ClawCore-specific tools
...
2. **Custom Replacements**: ClawCore replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **ClawCore Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures ClawCore's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, ClawCore CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
ClawCore wraps this with `guardSessionManager()` for tool result safety.
...
ClawCore maintains an auth profile store with multiple API keys per provider:
...
ClawCore loads custom pi extensions for specialized behavior:
...
ClawCore also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | ClawCore Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom ClawCore tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.clawcore/agents/<agentId>/sessions/` (or `$CLAWCORE_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-tools.create-clawcore-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-clawcore-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-clawcore-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-clawcore-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
