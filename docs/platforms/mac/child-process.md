...
service via the external `clawcore` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`CLAWCORE_PROFILE`; legacy `com.clawcore.*` is supported).
