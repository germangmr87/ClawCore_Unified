---
summary: "ClawCore on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up ClawCore on a Raspberry Pi
  - Running ClawCore on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# ClawCore on Raspberry Pi

## Goal

Run a persistent, always-on ClawCore Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most ClawCore features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled clawcore

# Enable if not
sudo systemctl enable clawcore

# Start on boot
sudo systemctl start clawcore
```
...
```bash
# Check logs
journalctl -u clawcore --no-pager -n 100

# Common fix: rebuild
cd ~/clawcore  # if using hackable install
npm run build
sudo systemctl restart clawcore
```
