---
summary: "Use MiniMax M2.1 in ClawCore"
read_when:
  - You want MiniMax models in ClawCore
...
```bash
clawcore plugins enable minimax-portal-auth  # skip if already loaded.
clawcore gateway restart  # restart if gateway is already running
clawcore onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/clawcore/clawcore/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `clawcore.json`:
...
- Use `clawcore models list` and `clawcore models set minimax/MiniMax-M2.1` to switch.
...
- Running `clawcore configure` and selecting **MiniMax M2.1**, or
