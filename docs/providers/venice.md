---
summary: "Use Venice AI privacy-focused models in ClawCore"
read_when:
  - You want privacy-focused inference in ClawCore
...
```bash
clawcore onboard --auth-choice venice-api-key
```
...
```bash
clawcore chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, ClawCore shows all available Venice models. Pick based on your needs:
...
```bash
clawcore models set venice/claude-opus-45
clawcore models set venice/llama-3.3-70b
```

List all available models:

```bash
clawcore models list | grep venice
```

## Configure via `clawcore configure`

1. Run `clawcore configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
ClawCore automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
clawcore chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
clawcore chat --model venice/claude-opus-45

# Use uncensored model
clawcore chat --model venice/venice-uncensored

# Use vision model with image
clawcore chat --model venice/qwen3-vl-235b-a22b

# Use coding model
clawcore chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `clawcore models list` to see currently available models. Some models may be temporarily offline.
