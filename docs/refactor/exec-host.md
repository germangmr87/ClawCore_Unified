...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.clawcore/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.clawcore/node.json` (node id + pairing token).
...
Path: `~/.clawcore/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.clawcore/exec-approvals.sock",
...
- Unix socket at `~/.clawcore/exec-approvals.sock` (0600).
