...
- Publish as `clawcore/plugin-sdk` (or export from core under `clawcore/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `ClawCorePluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ClawCoreConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ClawCoreConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ClawCoreConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ClawCoreConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ClawCoreConfig): string;
  };
};
```
...
- Introduce `clawcore/plugin-sdk`.
- Add `api.runtime` to `ClawCorePluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `clawcoreRuntime: ">=2026.2.0"`).
