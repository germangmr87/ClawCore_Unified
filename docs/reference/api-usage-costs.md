...
ClawCore features that can generate provider usage or paid API calls.
...
- `clawcore status --usage` and `clawcore channels list` show provider **usage windows**
...
ClawCore can pick up credentials from:
...
- `clawcore status --usage`
- `clawcore models status --json`
...
`clawcore models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
