...
ClawCore integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- ClawCore owns lifecycle when `channels.signal.autoStart=true`.
...
- ClawCore spawns `imsg rpc` as a child process (legacy iMessage integration).
