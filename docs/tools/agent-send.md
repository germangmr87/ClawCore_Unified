---
summary: "Direct `clawcore agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `clawcore agent` (direct agent runs)

`clawcore agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `clawcore message --target`).
...
```bash
clawcore agent --to +15555550123 --message "status update"
...
clawcore agent --to +15555550123 --message "Summon reply" --deliver
