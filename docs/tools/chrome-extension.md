---
summary: "Chrome extension: let ClawCore drive your existing Chrome tab"
read_when:
...
The ClawCore Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate clawcore-managed Chrome profile.
...
ClawCore then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the ClawCore release (npm package) as static files. There is no separate “build” step.

After upgrading ClawCore:

- Re-run `clawcore browser extension install` to refresh the installed files under your ClawCore state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

ClawCore ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want ClawCore to control.
...
- `ON`: attached; ClawCore can drive that tab.
...
Debugging: `clawcore sandbox explain`
...
`clawcore browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `clawcore browser extension install` first to copy the extension to a stable location under your ClawCore state directory.
...
- **This is not isolated** like the dedicated clawcore-managed profile.
