...
- **gateway host** → `clawcore` process on the gateway machine
...
`~/.clawcore/exec-approvals.json` directly.

CLI: `clawcore approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
