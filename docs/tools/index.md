---
summary: "Agent tool surface for ClawCore (browser, canvas, nodes, message, cron) replacing legacy `clawcore-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `clawcore-*` skills
title: "Tools"
---

# Tools (ClawCore)

ClawCore exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `clawcore-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `clawcore.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, ClawCore logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:clawcore`: all built-in ClawCore tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`clawcore node run`).
...
- Requires a Brave API key (recommended: `clawcore configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated ClawCore-managed browser.
...
- Quick smoke: `clawcore nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `clawcore gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, ClawCore clamps `tools.sessions.visibility` to `tree`.
