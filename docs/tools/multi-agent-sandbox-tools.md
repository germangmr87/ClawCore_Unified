...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `clawcore sandbox explain`.
...
Legacy `agent.*` configs are migrated by `clawcore doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=clawcore-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${CLAWCORE_STATE_DIR:-$HOME/.clawcore}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
