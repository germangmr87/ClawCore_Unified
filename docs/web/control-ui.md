...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/clawcore`)
...
If the page fails to load, start the Gateway first: `clawcore gateway`.
...
you revoke it with `clawcore devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.clawcore/clawcore.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. ClawCore
...
ClawCore **blocks** Control UI connections without device identity.
