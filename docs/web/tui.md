...
```bash
clawcore tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `clawcore tui --deliver`
...
- Check the Gateway logs: `clawcore logs --follow`.
- Confirm the agent can run: `clawcore status` and `clawcore models status`.
...
- No agents in picker: check `clawcore agents list` and your routing config.
