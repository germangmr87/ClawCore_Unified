...
你可以在 `~/.clawcore/clawcore.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/clawcore.log"
  }
}
```
...
```bash
clawcore channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.clawcore/clawcore.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/clawcore/clawcore-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- ClawCore 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
CLAWCORE_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `clawcore plugins enable diagnostics-otel` 启用插件。
...
- `clawcore.tokens`（计数器，属性：`clawcore.token`、`clawcore.channel`、`clawcore.provider`、`clawcore.model`）
- `clawcore.cost.usd`（计数器，属性：`clawcore.channel`、`clawcore.provider`、`clawcore.model`）
- `clawcore.run.duration_ms`（直方图，属性：`clawcore.channel`、`clawcore.provider`、`clawcore.model`）
- `clawcore.context.tokens`（直方图，属性：`clawcore.context`、`clawcore.channel`、`clawcore.provider`、`clawcore.model`）

消息流：

- `clawcore.webhook.received`（计数器，属性：`clawcore.channel`、`clawcore.webhook`）
- `clawcore.webhook.error`（计数器，属性：`clawcore.channel`、`clawcore.webhook`）
- `clawcore.webhook.duration_ms`（直方图，属性：`clawcore.channel`、`clawcore.webhook`）
- `clawcore.message.queued`（计数器，属性：`clawcore.channel`、`clawcore.source`）
- `clawcore.message.processed`（计数器，属性：`clawcore.channel`、`clawcore.outcome`）
- `clawcore.message.duration_ms`（直方图，属性：`clawcore.channel`、`clawcore.outcome`）

队列 + 会话：

- `clawcore.queue.lane.enqueue`（计数器，属性：`clawcore.lane`）
- `clawcore.queue.lane.dequeue`（计数器，属性：`clawcore.lane`）
- `clawcore.queue.depth`（直方图，属性：`clawcore.lane` 或 `clawcore.channel=heartbeat`）
- `clawcore.queue.wait_ms`（直方图，属性：`clawcore.lane`）
- `clawcore.session.state`（计数器，属性：`clawcore.state`、`clawcore.reason`）
- `clawcore.session.stuck`（计数器，属性：`clawcore.state`）
- `clawcore.session.stuck_age_ms`（直方图，属性：`clawcore.state`）
- `clawcore.run.attempt`（计数器，属性：`clawcore.attempt`）

### 导出的 span（名称 + 关键属性）

- `clawcore.model.usage`
  - `clawcore.channel`、`clawcore.provider`、`clawcore.model`
  - `clawcore.sessionKey`、`clawcore.sessionId`
  - `clawcore.tokens.*`（input/output/cache_read/cache_write/total）
- `clawcore.webhook.processed`
  - `clawcore.channel`、`clawcore.webhook`、`clawcore.chatId`
- `clawcore.webhook.error`
  - `clawcore.channel`、`clawcore.webhook`、`clawcore.chatId`、`clawcore.error`
- `clawcore.message.processed`
  - `clawcore.channel`、`clawcore.outcome`、`clawcore.chatId`、`clawcore.messageId`、`clawcore.sessionKey`、`clawcore.sessionId`、`clawcore.reason`
- `clawcore.session.stuck`
  - `clawcore.state`、`clawcore.ageMs`、`clawcore.queueDepth`、`clawcore.sessionKey`、`clawcore.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `clawcore doctor`。
