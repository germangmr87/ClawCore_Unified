...
import { approveDevicePairing, listDevicePairing } from "clawcore/plugin-sdk";
...
function resolveGatewayPort(cfg: ClawCorePluginApi["config"]): number {
  const envRaw =
    process.env.CLAWCORE_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: ClawCorePluginApi): Promise<string | null> {
...
function resolveAuth(cfg: ClawCorePluginApi["config"]): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    process.env.CLAWCORE_GATEWAY_TOKEN?.trim() ||
    process.env.CLAWDBOT_GATEWAY_TOKEN?.trim() ||
    cfg.gateway?.auth?.token?.trim();
  const password =
    process.env.CLAWCORE_GATEWAY_PASSWORD?.trim() ||
...
async function resolveGatewayUrl(api: ClawCorePluginApi): Promise<ResolveUrlResult> {
