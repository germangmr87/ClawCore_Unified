...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as ClawCoreConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as ClawCoreConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as ClawCoreConfig);
