...
`clawcore.invoke --each`) without adding new ClawCore code per workflow.
...
This extension depends on ClawCore internal modules (the embedded agent runner).
It is intended to ship as a **bundled** ClawCore extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.clawcore/extensions` as a standalone plugin directory.
