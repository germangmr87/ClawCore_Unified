...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["clawcorePollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["clawcore", "pollId"]) ??
    readNestedString(value, ["clawcore", "poll", "id"]) ??
    readNestedString(value, ["data", "clawcorePollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "clawcore", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          clawcorePollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "clawcore poll vote",
          displayText: "Vote recorded",
          value: { clawcorePollId: pollId, pollId },
        },
      },
    ],
  };
