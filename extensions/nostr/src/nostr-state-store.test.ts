import type { PluginRuntime } from "clawcore/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.CLAWCORE_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-nostr-"));
  process.env.CLAWCORE_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.CLAWCORE_STATE_DIR?.trim() || env.CLAWCORE_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".clawcore");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.CLAWCORE_STATE_DIR = previous;
    }
