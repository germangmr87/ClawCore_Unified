...
metadata: { "clawcore": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == ClawCore `sessions_spawn`
- **File I/O** == ClawCore `read`/`write`
- **Remote fetch** == ClawCore `web_fetch` (or `exec` with curl when POST is required)
