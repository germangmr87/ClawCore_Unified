import type { ClawCorePluginApi, ClawCorePluginService } from "clawcore/plugin-sdk";
...
function normalizeDenyList(cfg: ClawCorePluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: ClawCorePluginApi["config"]): string[] {
...
export default function register(api: ClawCorePluginApi) {
  ...
  const timerService: ClawCorePluginService = {
