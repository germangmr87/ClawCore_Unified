...
const meta = (0, plugin_sdk_1.getChatChannelMeta)("telegram");
const telegramMessageActions = {
    listActions: (ctx) => (0, runtime_js_1.getTelegramRuntime)().channel.telegram.messageActions?.listActions?.(ctx) ?? [],
    extractToolSend: (ctx) => (0, runtime_js_1.getTelegramRuntime)().channel.telegram.messageActions?.extractToolSend?.(ctx) ?? null,
    handleAction: async (ctx) => {
        const ma = (0, runtime_js_1.getTelegramRuntime)().channel.telegram.messageActions;
        if (!ma?.handleAction) {
...
exports.telegramPlugin = {
    ...
    pairing: {
        ...
        notifyApproval: async ({ cfg, id }) => {
            const { token } = (0, runtime_js_1.getTelegramRuntime)().channel.telegram.resolveTelegramToken(cfg);
            if (!token) {
                throw new Error("telegram token not configured");
            }
            await (0, runtime_js_1.getTelegramRuntime)().channel.telegram.sendMessageTelegram(id, plugin_sdk_1.PAIRING_APPROVED_MESSAGE, {
                token,
            });
        },
    },
    ...
    config: {
        listAccountIds: (cfg) => (0, plugin_sdk_1.listTelegramAccountIds)(cfg),
        resolveAccount: (cfg, accountId) => (0, plugin_sdk_1.resolveTelegramAccount)({ cfg, accountId }),
        defaultAccountId: (cfg) => (0, plugin_sdk_1.resolveDefaultTelegramAccountId)(cfg),
        ...
        resolveAllowFrom: ({ cfg, accountId }) => ((0, plugin_sdk_1.resolveTelegramAccount)({ cfg, accountId }).config.allowFrom ?? []).map((entry) => String(entry)),
    ...
    directory: {
        self: async () => null,
        listPeers: async (params) => (0, plugin_sdk_1.listTelegramDirectoryPeersFromConfig)(params),
        listGroups: async (params) => (0, plugin_sdk_1.listTelegramDirectoryGroupsFromConfig)(params),
    },
    ...
    outbound: {
        deliveryMode: "direct",
        chunker: (text, limit) => (0, runtime_js_1.getTelegramRuntime)().channel.text.chunkMarkdownText(text, limit),
        chunkerMode: "markdown",
        textChunkLimit: 4000,
        pollMaxOptions: 10,
        sendText: async ({ to, text, accountId, deps, replyToId, threadId, silent }) => {
            const send = deps?.sendTelegram ?? (0, runtime_js_1.getTelegramRuntime)().channel.telegram.sendMessageTelegram;
            const replyToMessageId = (0, plugin_sdk_1.parseTelegramReplyToMessageId)(replyToId);
            const messageThreadId = (0, plugin_sdk_1.parseTelegramThreadId)(threadId);
        ...
        sendPoll: async ({ to, poll, accountId, threadId, silent, isAnonymous }) => await (0, runtime_js_1.getTelegramRuntime)().channel.telegram.sendPollTelegram(to, poll, {
            accountId: accountId ?? undefined,
            messageThreadId: (0, plugin_sdk_1.parseTelegramThreadId)(threadId),
            silent: silent ?? undefined,
    ...
    status: {
        ...
        probeAccount: async ({ account, timeoutMs }) => (0, runtime_js_1.getTelegramRuntime)().channel.telegram.probeTelegram(account.token, timeoutMs, account.config.proxy),
        auditAccount: async ({ account, timeoutMs, probe, cfg }) => {
            const groups = cfg.channels?.telegram?.accounts?.[account.accountId]?.groups ??
                cfg.channels?.telegram?.groups;
            const { groupIds, unresolvedGroups, hasWildcardUnmentionedGroups } = (0, runtime_js_1.getTelegramRuntime)().channel.telegram.collectUnmentionedGroupIds(groups);
            ...
            const audit = await (0, runtime_js_1.getTelegramRuntime)().channel.telegram.auditGroupMembership({
                token: account.token,
                botId,
                groupIds,
    ...
    gateway: {
        startAccount: async (ctx) => {
            ...
            try {
                const probe = await (0, runtime_js_1.getTelegramRuntime)().channel.telegram.probeTelegram(token, 2500, account.config.proxy);
            ...
            return (0, runtime_js_1.getTelegramRuntime)().channel.telegram.monitorTelegramProvider({
                token,
                accountId: account.accountId,
                config: ctx.cfg,
                runtime: ctx.runtime,
        ...
        logoutAccount: async ({ accountId, cfg }) => {
            ...
            const resolved = (0, plugin_sdk_1.resolveTelegramAccount)({
                cfg: changed ? nextCfg : cfg,
                accountId,
            });
            const loggedOut = resolved.tokenSource === "none";
            if (changed) {
                await (0, runtime_js_1.getTelegramRuntime)().config.writeConfigFile(nextCfg);
            }
