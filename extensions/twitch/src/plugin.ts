/**
 * Twitch channel plugin for ClawCore.
 *
...
import { buildChannelConfigSchema } from "clawcore/plugin-sdk";
import type {
 ...
 * for ClawCore. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: ClawCoreConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: ClawCoreConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: ClawCoreConfig): boolean => {
