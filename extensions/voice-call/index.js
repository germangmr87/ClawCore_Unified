...
const voiceCallConfigSchema = {
    parse(value) {
        ...
        return config_js_1.VoiceCallConfigSchema.parse({
            ...raw,
            enabled,
            provider,
...
const VoiceCallToolSchema = typebox_1.Type.Union([
...
const voiceCallPlugin = {
    ...
    register(api) {
        const config = (0, config_js_1.resolveVoiceCallConfig)(voiceCallConfigSchema.parse(api.pluginConfig));
        ...
        api.registerGatewayMethod("voicecall.status", async ({ params, respond }) => {
            try {
                const raw = typeof params?.callId === "string"
                    ? params.callId.trim()
        ...
        api.registerCli(({ program }) => (0, cli_js_1.registerVoiceCallCli)({
            program,
            config,
            ensureRuntime,
            logger: api.logger,
