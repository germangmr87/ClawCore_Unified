...
function resolveDefaultStorePath(config) {
    ...
    const resolvedPreferred = (0, utils_js_1.resolveUserPath)(preferred);
    ...
    const base = config.store?.trim() ? (0, utils_js_1.resolveUserPath)(config.store) : existing;
...
function registerVoiceCallCli(params) {
    ...
    const root = program
        ...
        .addHelpText("after", () => `\nDocs: https://docs.clawcore.ai/cli/voicecall\n`);
    root
        .command("call")
    ...
    root
        ...
        const initial = node_fs_1.default.readFileSync(file, "utf8");
        ...
        for (;;) {
            ...
            await (0, plugin_sdk_1.sleep)(pollMs);
        }
    });
    root
        .command("expose")
        ...
        if (mode === "off") {
            await (0, webhook_js_1.cleanupTailscaleExposureRoute)({ mode: "serve", path: tsPath });
            await (0, webhook_js_1.cleanupTailscaleExposureRoute)({ mode: "funnel", path: tsPath });
            // eslint-disable-next-line no-console
            console.log(JSON.stringify({ ok: true, mode: "off", path: tsPath }, null, 2));
            return;
        }
        const publicUrl = await (0, webhook_js_1.setupTailscaleExposureRoute)({
            mode,
            path: tsPath,
            localUrl,
        });
        const tsInfo = publicUrl ? null : await (0, webhook_js_1.getTailscaleSelfInfo)();
