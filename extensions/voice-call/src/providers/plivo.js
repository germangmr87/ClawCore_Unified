...
class PlivoProvider {
    ...
    verifyWebhook(ctx) {
        const result = (0, webhook_security_js_1.verifyPlivoWebhook)(ctx, this.authToken, {
...
<Response>
  <Wait length="300" />
    ...
    static xmlSpeak(text, locale) {
        ...
        return `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Speak language="${(0, voice_mapping_js_1.escapeXml)(language)}">${(0, voice_mapping_js_1.escapeXml)(text)}</Speak>
  <Wait length="300" />
</Response>`;
    }
    static xmlGetInputSpeech(params) {
        ...
        return `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <GetInput inputType="speech" method="POST" action="${(0, voice_mapping_js_1.escapeXml)(params.actionUrl)}" language="${(0, voice_mapping_js_1.escapeXml)(language)}" executionTimeout="30" speechEndTimeout="1" redirect="false">
  </GetInput>
  <Wait length="300" />
</Response>`;
    }
    ...
    baseWebhookUrlFromCtx(ctx) {
        try {
            const u = new URL((0, webhook_security_js_1.reconstructWebhookUrl)(ctx, {
                allowedHosts: this.options.webhookSecurity?.allowedHosts,
                trustForwardingHeaders: this.options.webhookSecurity?.trustForwardingHeaders,
                trustedProxyIPs: this.options.webhookSecurity?.trustedProxyIPs,
                remoteIP: ctx.remoteAddress,
