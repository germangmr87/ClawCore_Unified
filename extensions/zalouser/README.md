...
ClawCore extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
clawcore channels login --channel zalouser
...
```bash
clawcore message send --channel zalouser --target <threadId> --message "Hello from ClawCore!"
```
...
```bash
clawcore channels login --channel zalouser              # Login via QR
clawcore channels login --channel zalouser --account work
clawcore channels status --probe
clawcore channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
clawcore directory self --channel zalouser
clawcore directory peers list --channel zalouser --query "name"
clawcore directory groups list --channel zalouser --query "work"
clawcore directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
clawcore message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
clawcore message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `clawcore channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
clawcore channels login --channel zalouser --account work
clawcore message send --channel zalouser --account work --target <id> --message "Hello"
