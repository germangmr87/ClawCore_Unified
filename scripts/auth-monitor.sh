...
#
# Suggested cron: */30 * * * * /home/admin/clawcore/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send ClawCore notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., clawcore-alerts)
...
STATE_FILE="$HOME/.clawcore/auth-monitor-state"
...
send_notification() {
    ...
    # Send via ClawCore if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use clawcore
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via ClawCore to $NOTIFY_PHONE..."
            clawcore send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! ClawCore is down. Run: ssh l36 '~/clawcore/scripts/mobile-reauth.sh'" "urgent"
    exit 1
