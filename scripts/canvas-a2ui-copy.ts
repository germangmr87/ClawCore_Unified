...
export function getA2uiPaths(env = process.env) {
  const srcDir = env.CLAWCORE_A2UI_SRC_DIR ?? path.join(repoRoot, "src", "canvas-host", "a2ui");
  const outDir = env.CLAWCORE_A2UI_OUT_DIR ?? path.join(repoRoot, "dist", "canvas-host", "a2ui");
