...
INSTALL_URL="${CLAWCORE_INSTALL_URL:-https://clawcore.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${CLAWCORE_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${CLAWCORE_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="clawcore"
PACKAGE_NAME="${CLAWCORE_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (CLAWCORE_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${CLAWCORE_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${CLAWCORE_INSTALL_LATEST_OUT:-}"
fi
