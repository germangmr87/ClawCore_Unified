...
IMAGE_NAME="clawcore-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="clawcore-net-e2e-$$"
GW_NAME="clawcore-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "CLAWCORE_GATEWAY_TOKEN=$TOKEN" \
	  -e "CLAWCORE_SKIP_CHANNELS=1" \
	  -e "CLAWCORE_SKIP_GMAIL_WATCHER=1" \
	  -e "CLAWCORE_SKIP_CRON=1" \
	  -e "CLAWCORE_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
