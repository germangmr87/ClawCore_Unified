...
IMAGE_NAME="clawcore-plugins-e2e"
...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  if [ -f dist/index.mjs ]; then
	    CLAWCORE_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    CLAWCORE_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export CLAWCORE_ENTRY

	  home_dir=$(mktemp -d "/tmp/clawcore-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.clawcore/extensions/demo-plugin"

  cat > "$HOME/.clawcore/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.clawcore/extensions/demo-plugin/clawcore.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$CLAWCORE_ENTRY" plugins list --json > /tmp/plugins.json
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/clawcore-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@clawcore/demo-plugin-tgz",
  "version": "0.0.1",
...
JS
  cat > "$pack_dir/package/clawcore.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$CLAWCORE_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$CLAWCORE_ENTRY" plugins list --json > /tmp/plugins2.json
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/clawcore-plugin-dir.XXXXXX")"
...
JS
  cat > "$dir_plugin/clawcore.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$CLAWCORE_ENTRY" plugins install "$dir_plugin"
	  node "$CLAWCORE_ENTRY" plugins list --json > /tmp/plugins3.json
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/clawcore-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@clawcore/demo-plugin-file",
  "version": "0.0.1",
  "clawcore": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/clawcore.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$CLAWCORE_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$CLAWCORE_ENTRY" plugins list --json > /tmp/plugins4.json
