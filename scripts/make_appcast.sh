...
ZIP=${1:?"Usage: $0 ClawCore-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/clawcore/clawcore/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^ClawCore-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/clawcore/clawcore/releases/download/v${VERSION}/"}
