...
# Build and bundle ClawCore into a minimal .app we can open.
# Outputs to dist/ClawCore.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/ClawCore.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="ClawCore"
BUNDLE_ID="${BUNDLE_ID:-ai.clawcore.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/clawcore/clawcore/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/ClawCore/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ClawCoreBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ClawCoreGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/ClawCore"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/ClawCore"
fi
chmod +x "$APP_ROOT/Contents/MacOS/ClawCore"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/ClawCore" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/ClawCore/Resources/ClawCore.icns" "$APP_ROOT/Contents/Resources/ClawCore.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/ClawCore/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ClawCoreKit resources"
CLAWCOREKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ClawCoreKit_ClawCoreKit.bundle"
if [ -d "$CLAWCOREKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ClawCoreKit_ClawCoreKit.bundle"
  cp -R "$CLAWCOREKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ClawCoreKit_ClawCoreKit.bundle"
else
  echo "WARN: ClawCoreKit resource bundle not found at $CLAWCOREKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running ClawCore"
killall -q ClawCore 2>/dev/null || true
