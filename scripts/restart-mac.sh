#!/usr/bin/env bash
# Reset ClawCore like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${CLAWCORE_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="ClawCore.app/Contents/MacOS/ClawCore"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/ClawCore"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/ClawCore"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/ClawCore"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.clawcore.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/clawcore-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${CLAWCORE_RESTART_LOG:-/tmp/clawcore-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${CLAWCORE_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.clawcore/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  CLAWCORE_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node clawcore.mjs daemon install --force --runtime node"
      log "  node clawcore.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.clawcore/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.clawcore.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing ClawCore instances"
kill_all_clawcore
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product ClawCore"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/ClawCore.app" ]]; then
    APP_BUNDLE="/Applications/ClawCore.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/ClawCore.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/ClawCore.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/ClawCore.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set CLAWCORE_APP_BUNDLE to your installed ClawCore.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node clawcore.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node clawcore.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".clawcore", "clawcore.json"), "utf8");
