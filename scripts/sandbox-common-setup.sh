...
BASE_IMAGE="${BASE_IMAGE:-clawcore-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-clawcore-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=clawcore.sandbox=1)
NOTE
