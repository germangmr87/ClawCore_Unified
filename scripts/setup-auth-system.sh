#!/bin/bash
# Setup ClawCore Auth Management System
...
echo "=== ClawCore Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'clawcore-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  ClawCore message: Send warning via ClawCore itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/clawcore-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/clawcore-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/clawcore-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now clawcore-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status clawcore-auth-monitor.timer"
echo ""
echo "Quick commands:"
