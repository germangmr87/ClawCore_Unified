#!/usr/bin/env bash
# ClawDock - Docker helpers for ClawCore
# Inspired by Simon Willison's "Running ClawCore in Docker"
# https://til.simonwillison.net/llms/clawcore-docker
#
# Installation:
...
# Common paths to check for ClawCore
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/clawcore"
  "${HOME}/workspace/clawcore"
  "${HOME}/projects/clawcore"
  "${HOME}/dev/clawcore"
  "${HOME}/code/clawcore"
  "${HOME}/src/clawcore"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found ClawCore at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/clawcore"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ ClawCore not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/clawcore/clawcore.git ~/clawcore"
    echo "  cd ~/clawcore && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^CLAWCORE_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d clawcore-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart clawcore-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f clawcore-gateway
}
...
clawdock-workspace() {
  cd ~/.clawcore/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec clawcore-gateway \
    bash -c 'echo "alias clawcore=\"./clawcore.mjs\"" > /tmp/.bashrc_clawcore && bash --rcfile /tmp/.bashrc_clawcore'
}
...
clawdock-cli() {
  _clawdock_compose run --rm clawcore-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build clawcore-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "CLAWCORE_GATEWAY_TOKEN=$token" clawcore-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" clawcore-gateway \
    bash -c './clawcore.mjs config set gateway.remote.token "$TOKEN" && ./clawcore.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec clawcore-gateway \
    bash -c "./clawcore.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart clawcore-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm clawcore-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec clawcore-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'clawcore config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for ClawCore${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (clawcore alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to clawcore project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.clawcore)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'clawcore channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'clawcore status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.clawcore.ai${_CLR_RESET}"
  echo ""
}
