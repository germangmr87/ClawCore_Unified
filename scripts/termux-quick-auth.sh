...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/clawcore/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "ClawCore Re-Auth" -c "After getting key, run: ssh $SERVER '~/clawcore/scripts/mobile-reauth.sh'" --id clawcore-auth
        ;;
    *)
