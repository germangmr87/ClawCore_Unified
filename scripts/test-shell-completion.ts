 ...
 * `clawcore update`. Use it to verify the completion installation flow
...
\`clawcore update\`, \`clawcore doctor\`, and \`clawcore onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# ClawCore Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`clawcore completion --install\` later to enable.`));
    return;
  }
