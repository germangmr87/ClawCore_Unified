---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for ClawCore deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, ClawCore cron scheduling for periodic checks, or version status checks on a machine running ClawCore (laptop, workstation, Pi, VPS).
---

# ClawCore Host Hardening

## Overview

Assess and harden the host running ClawCore, then align it to a user-defined risk tolerance without breaking access. Use ClawCore security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim ClawCore changes the host firewall, SSH, or OS updates; it does not.
...
5. ClawCore gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if ClawCore can access sensitive data.
...
### 2) Run ClawCore security audits (read-only)

As part of the default read-only checks, run `clawcore security audit --deep`. Only offer alternatives if the user requests them:

1. `clawcore security audit` (faster, non-probing)
2. `clawcore security audit --json` (structured output)

Offer to apply ClawCore safe defaults (numbered):

1. `clawcore security audit --fix`

Be explicit that `--fix` only tightens ClawCore defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check ClawCore version/update status (read-only)

As part of the default read-only checks, run `clawcore update status`.
...
- Credential hygiene notes (location of ClawCore creds, prefer disk encryption)
...
- ClawCore security audit (re-run)
...
After ClawCore install or first hardening pass, run at least one baseline audit and version check:

- `clawcore security audit`
- `clawcore security audit --deep`
- `clawcore update status`

Ongoing monitoring is recommended. Use the ClawCore cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `clawcore cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `clawcore update status`
...
Before creating, `clawcore cron list` and match on exact `name`. If found, `clawcore cron edit <id> ...`.
If not found, `clawcore cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `clawcore update status` (preferred for source checkouts and channels)
2. `npm view clawcore version` (published npm version)

## ClawCore command accuracy

Use only supported commands and flags:

- `clawcore security audit [--deep] [--fix] [--json]`
- `clawcore status` / `clawcore status --deep`
- `clawcore health --json`
- `clawcore update status`
- `clawcore cron add|list|runs|run`

Do not invent CLI flags or imply ClawCore enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by ClawCore compaction:
