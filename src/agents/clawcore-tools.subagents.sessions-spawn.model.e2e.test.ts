...
} from "./clawcore-tools.subagents.sessions-spawn.test-harness.js";
import { resetSubagentRegistryForTests } from "./subagent-registry.js";

const callGatewayMock = getCallGatewayMock();

type CreateClawCoreTools = (typeof import("./clawcore-tools.js"))["createClawCoreTools"];
type CreateClawCoreToolsOpts = Parameters<CreateClawCoreTools>[0];

async function getSessionsSpawnTool(opts: CreateClawCoreToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createClawCoreTools } = await import("./clawcore-tools.js");
  const tool = createClawCoreTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("clawcore-tools: subagents (sessions_spawn model + thinking)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
