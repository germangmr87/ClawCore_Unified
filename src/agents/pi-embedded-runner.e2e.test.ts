...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-embedded-agent-"));
...
const makeOpenAiConfig = (modelIds: string[]) =>
  ...
  }) satisfies ClawCoreConfig;

const ensureModels = (cfg: ClawCoreConfig) => ensureClawCoreModelsJson(cfg, agentDir) as unknown;
