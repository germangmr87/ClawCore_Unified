...
import { resolveClawCoreDocsPath } from "../docs-path.js";
...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolveClawCoreAgentDir();
  await ensureClawCoreModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createClawCoreCodingTools({
      exec: {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolveClawCoreDocsPath({
      workspaceDir: effectiveWorkspace,
