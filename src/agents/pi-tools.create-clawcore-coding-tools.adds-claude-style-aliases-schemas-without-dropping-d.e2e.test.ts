...
describe("createClawCoreCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from clawcore read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createClawCoreCodingTools({ sandbox });
