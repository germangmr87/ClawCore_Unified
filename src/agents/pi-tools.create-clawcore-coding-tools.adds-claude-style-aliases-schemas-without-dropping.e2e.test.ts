...
import { createClawCoreTools } from "./clawcore-tools.js";
import { __testing, createClawCoreCodingTools } from "./pi-tools.js";
...
describe("createClawCoreCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createClawCoreTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createClawCoreCodingTools({ messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "clawcore-outside.txt");
