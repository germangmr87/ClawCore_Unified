...
function resolveExecConfig(params: { cfg?: ClawCoreConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: ClawCoreConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createClawCoreCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createClawCoreReadTool(freshReadTool);
