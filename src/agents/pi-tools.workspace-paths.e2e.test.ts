...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("clawcore-ws-", async (workspaceDir) => {
      await withTempDir("clawcore-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createClawCoreCodingTools({ workspaceDir });
    ...
    await withTempDir("clawcore-ws-", async (workspaceDir) => {
      await withTempDir("clawcore-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello clawcore");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("clawcore-ws-", async (workspaceDir) => {
      await withTempDir("clawcore-override-", async (overrideDir) => {
        const tools = createClawCoreCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("clawcore-sandbox-", async (sandboxDir) => {
      await withTempDir("clawcore-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createClawCoreCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
