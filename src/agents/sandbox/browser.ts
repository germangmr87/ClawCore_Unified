...
import {
  ...
  DEFAULT_CLAWCORE_BROWSER_COLOR,
  DEFAULT_CLAWCORE_BROWSER_PROFILE_NAME,
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_CLAWCORE_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_CLAWCORE_BROWSER_PROFILE_NAME,
    profiles: {
      [DEFAULT_CLAWCORE_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "clawcore.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `clawcore sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `clawcore sandbox recreate --browser --agent ${agentId}`;
          }
          return "clawcore sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: { "clawcore.sandboxBrowser": "1" },
    ...
    args.push("-e", `CLAWCORE_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `CLAWCORE_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `CLAWCORE_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `CLAWCORE_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `CLAWCORE_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_CLAWCORE_BROWSER_PROFILE_NAME)
    : null;
