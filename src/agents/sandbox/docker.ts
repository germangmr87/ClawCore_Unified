...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "clawcore.sandbox=1");
  args.push("--label", `clawcore.sessionKey=${params.scopeKey}`);
  args.push("--label", `clawcore.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `clawcore.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "clawcore.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`clawcore sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`clawcore sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("clawcore sandbox recreate --all");
}
