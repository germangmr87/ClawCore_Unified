...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["CLAWCORE_STATE_DIR"]);
  ...
  it("persists runs to disk and resumes after restart", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-subagent-"));
  ...
  it("uses isolated temp state when CLAWCORE_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "clawcore-test-state"));
  });
});
