...
type WebSearchConfig = NonNullable<ClawCoreConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: ClawCoreConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("clawcore configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.clawcore.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://clawcore.ai",
      "X-Title": "ClawCore Web Search",
    },
