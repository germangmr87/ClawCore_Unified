...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses CLAWCORE_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      CLAWCORE_HOME: "/srv/clawcore-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/clawcore-home"), ".clawcore", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".clawcore", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("clawcore-workspace-");
