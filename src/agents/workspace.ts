...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.CLAWCORE_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".clawcore", `workspace-${profile}`);
  }
  return path.join(home, ".clawcore", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".clawcore";
