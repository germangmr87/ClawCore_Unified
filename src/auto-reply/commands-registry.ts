...
export function isCommandEnabled(cfg: ClawCoreConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: ClawCoreConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as ClawCoreConfig),
...
export function getCommandDetection(_cfg?: ClawCoreConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: ClawCoreConfig) {
