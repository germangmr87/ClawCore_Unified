...
import { scheduleGatewaySigusr1Restart, triggerClawCoreRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting ClawCore in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerClawCoreRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting ClawCore via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
