...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".clawcore-profile-decorated");
}
 ...
 */
export function decorateClawCoreProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_CLAWCORE_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_CLAWCORE_BROWSER_COLOR).toUpperCase();
