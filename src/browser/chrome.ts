...
export function resolveClawCoreUserDataDir(profileName = DEFAULT_CLAWCORE_BROWSER_PROFILE_NAME) {
...
export async function launchClawCoreChrome(
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_CLAWCORE_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateClawCoreProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  try {
    ...
    log.warn(`clawcore browser clean-exit prefs failed: ${String(err)}`);
  }
...
export async function stopClawCoreChrome(running: RunningChrome, timeoutMs = 2500) {
