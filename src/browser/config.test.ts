...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const clawcore = resolveProfile(resolved, "clawcore");
    expect(clawcore?.driver).toBe("clawcore");
    expect(clawcore?.cdpPort).toBe(18800);
    expect(clawcore?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from CLAWCORE_GATEWAY_PORT when unset", () => {
    const prev = process.env.CLAWCORE_GATEWAY_PORT;
    process.env.CLAWCORE_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const clawcore = resolveProfile(resolved, "clawcore");
      expect(clawcore?.cdpPort).toBe(19012);
      expect(clawcore?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.CLAWCORE_GATEWAY_PORT;
      } else {
        process.env.CLAWCORE_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const clawcore = resolveProfile(resolved, "clawcore");
      expect(clawcore?.cdpPort).toBe(19022);
      expect(clawcore?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.CLAWCORE_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "clawcore");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        clawcore: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("clawcore");
  });
});
