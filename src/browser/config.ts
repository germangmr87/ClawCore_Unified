import type { BrowserConfig, BrowserProfileConfig, ClawCoreConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_CLAWCORE_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "clawcore" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_CLAWCORE_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "clawcore" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_CLAWCORE_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_CLAWCORE_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an ClawCore browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for clawcore/clawcore CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: ClawCoreConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_CLAWCORE_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "clawcore";
