...
vi.mock("./chrome.js", () => ({
  resolveClawCoreUserDataDir: vi.fn(() => "/tmp/clawcore-test/clawcore/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveClawCoreUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "clawcore-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveClawCoreUserDataDir).mockReturnValue(userDataDir);
