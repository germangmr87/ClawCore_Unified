...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredClawCoreTmpDir: vi.fn(() => "/tmp/clawcore"),
}));
vi.mock("../infra/tmp-clawcore-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredClawCoreTmpDir.mockReturnValue("/tmp/clawcore");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    ...
    tmpDirMocks.resolvePreferredClawCoreTmpDir.mockReturnValue("/tmp/clawcore-preferred");
    setPwToolsCoreCurrentPage({ on, off });
    ...
    const expectedDownloadsTail = `${path.join("tmp", "clawcore-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredClawCoreTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "clawcore-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "clawcore-preferred", "downloads")}${path.sep}`),
    );
  });
