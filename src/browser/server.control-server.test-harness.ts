...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/clawcore" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "clawcore",
        profiles: {
          clawcore: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchClawCoreChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveClawCoreUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopClawCoreChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.CLAWCORE_GATEWAY_PORT;
    process.env.CLAWCORE_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.CLAWCORE_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.CLAWCORE_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.CLAWCORE_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.CLAWCORE_GATEWAY_TOKEN;
    } else {
      process.env.CLAWCORE_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.CLAWCORE_GATEWAY_PASSWORD;
    } else {
      process.env.CLAWCORE_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
