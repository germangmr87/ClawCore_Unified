...
describe("canvas host state dir defaults", () => {
  ...
  it("uses CLAWCORE_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("ClawCore Canvas");
    } finally {
      await handler.close();
