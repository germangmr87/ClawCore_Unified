...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.clawcoreCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.clawcoreCanvasA2UIAction &&
        typeof window.clawcoreCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.clawcoreSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("clawcore:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android ClawCore node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.clawcoreSendUserAction === "function"
        ? window.clawcoreSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.CLAWCORE_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.CLAWCORE_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>ClawCore Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
