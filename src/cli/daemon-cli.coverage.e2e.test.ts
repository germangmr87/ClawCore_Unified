...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    CLAWCORE_STATE_DIR: process.env.CLAWCORE_STATE_DIR,
    CLAWCORE_CONFIG_PATH: process.env.CLAWCORE_CONFIG_PATH,
    CLAWCORE_GATEWAY_PORT: process.env.CLAWCORE_GATEWAY_PORT,
    CLAWCORE_PROFILE: process.env.CLAWCORE_PROFILE,
  };

  beforeEach(() => {
    process.env.CLAWCORE_STATE_DIR = "/tmp/clawcore-cli-state";
    process.env.CLAWCORE_CONFIG_PATH = "/tmp/clawcore-cli-state/clawcore.json";
    delete process.env.CLAWCORE_GATEWAY_PORT;
    delete process.env.CLAWCORE_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.CLAWCORE_STATE_DIR !== undefined) {
      process.env.CLAWCORE_STATE_DIR = originalEnv.CLAWCORE_STATE_DIR;
    } else {
      delete process.env.CLAWCORE_STATE_DIR;
    }

    if (originalEnv.CLAWCORE_CONFIG_PATH !== undefined) {
      process.env.CLAWCORE_CONFIG_PATH = originalEnv.CLAWCORE_CONFIG_PATH;
    } else {
      delete process.env.CLAWCORE_CONFIG_PATH;
    }

    if (originalEnv.CLAWCORE_GATEWAY_PORT !== undefined) {
      process.env.CLAWCORE_GATEWAY_PORT = originalEnv.CLAWCORE_GATEWAY_PORT;
    } else {
      delete process.env.CLAWCORE_GATEWAY_PORT;
    }

    if (originalEnv.CLAWCORE_PROFILE !== undefined) {
      process.env.CLAWCORE_PROFILE = originalEnv.CLAWCORE_PROFILE;
    } else {
      delete process.env.CLAWCORE_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        CLAWCORE_PROFILE: "dev",
        CLAWCORE_STATE_DIR: "/tmp/clawcore-daemon-state",
        CLAWCORE_CONFIG_PATH: "/tmp/clawcore-daemon-state/clawcore.json",
        CLAWCORE_GATEWAY_PORT: "19001",
      },
