...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("clawcore node install"), formatCliCommand("clawcore node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("clawcore node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    CLAWCORE_LOG_PREFIX: baseEnv.CLAWCORE_LOG_PREFIX ?? "node",
