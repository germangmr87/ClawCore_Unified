...
${formatHelpExamples([
  ['clawcore agent --to +15555550123 --message "status update"', "Start a new session."],
  ['clawcore agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'clawcore agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'clawcore agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['clawcore agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.clawcore.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.clawcore.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "clawcore agents set-identity --identity-file ~/.clawcore/workspace/IDENTITY.md --agent main",
