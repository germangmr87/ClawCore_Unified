...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    envSnapshot = captureEnv(["CLAWCORE_BUNDLED_SKILLS_DIR"]);
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "clawcore-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "clawcore-bundled-skills-test-"));
    process.env.CLAWCORE_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
