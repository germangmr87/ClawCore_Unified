...
vi.mock("../infra/clawcore-root.js", () => ({
  resolveClawCorePackageRoot: vi.fn(),
}));
...
const { resolveClawCorePackageRoot } = await import("../infra/clawcore-root.js");
...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-update-tests-"));
  });
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("clawcore-update");
    setTty(false);
    readPackageVersion.mockResolvedValue("2.0.0");

    vi.mocked(resolveClawCorePackageRoot).mockResolvedValue(tempDir);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveClawCorePackageRoot).mockReset();
    ...
    vi.mocked(resolveClawCorePackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("clawcore");
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("ClawCore update status");
  });
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("clawcore-update-wizard");
    const envSnapshot = captureEnv(["CLAWCORE_GIT_DIR"]);
    try {
      setTty(true);
      process.env.CLAWCORE_GIT_DIR = tempDir;
