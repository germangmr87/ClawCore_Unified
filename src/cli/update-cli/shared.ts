...
import { resolveClawCorePackageRoot } from "../../infra/clawcore-root.js";
...
const CLAWCORE_REPO_URL = "https://github.com/clawcore/clawcore.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "clawcore";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("clawcore@")) {
    return trimmed.slice("clawcore@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.CLAWCORE_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveClawCorePackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", CLAWCORE_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `CLAWCORE_GIT_DIR points at a non-git directory: ${params.dir}. Set CLAWCORE_GIT_DIR to an empty folder or an clawcore checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`CLAWCORE_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "clawcore.mjs");
