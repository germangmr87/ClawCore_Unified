...
export function listAgentEntries(cfg: ClawCoreConfig): AgentEntry[] {
...
function resolveAgentName(cfg: ClawCoreConfig, agentId: string) {
...
function resolveAgentModel(cfg: ClawCoreConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ClawCoreConfig): AgentSummary[] {
