...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    expect(resolveGatewayDevMode(["node", "/Users/me/clawcore/src/cli/index.js"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\clawcore\\src\\cli\\index.js"])).toBe(
      true,
    );
    expect(resolveGatewayDevMode(["node", "/Users/me/clawcore/dist/cli/index.js"])).toBe(false);
  });
});
...
describe("buildGatewayInstallPlan", () => {
  it("uses provided nodePath and returns plan", async () => {
    ...
    mocks.buildServiceEnvironment.mockReturnValue({ CLAWCORE_PORT: "3000" });
    ...
    expect(plan.environment).toEqual({ CLAWCORE_PORT: "3000" });
  ...
  it("merges config env vars into the environment", async () => {
    ...
    expect(plan.environment.CLAWCORE_PORT).toBe("3000");
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:clawcore|clawcore)( --profile isolated)? gateway install/,
    );
  });
});
