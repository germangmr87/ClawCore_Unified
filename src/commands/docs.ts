...
const SEARCH_TOOL = "https://docs.clawcore.ai/mcp.SearchClawCore";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.clawcore.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('clawcore docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.clawcore.ai/");
      runtime.log(`Search: ${formatCliCommand('clawcore docs "your query"')}`);
    }
    return;
  }
