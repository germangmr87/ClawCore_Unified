...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "CLAWCORE_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("CLAWCORE_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv CLAWCORE_GATEWAY_TOKEN");
    expect(message).not.toContain("CLAWCORE_GATEWAY_PASSWORD");
  });
