...
import type { ClawCoreConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: ClawCoreConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("clawcore security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("clawcore configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("clawcore config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("clawcore doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("clawcore config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('clawcore config set session.dmScope "per-channel-peer"') +
