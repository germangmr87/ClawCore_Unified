...
export const resolveClawCorePackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadClawCorePlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/clawcore-root.js", () => ({
  resolveClawCorePackageRoot,
}));
...
beforeEach(() => {
  ...
  resolveClawCorePackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.CLAWCORE_STATE_DIR;
  originalUpdateInProgress = process.env.CLAWCORE_UPDATE_IN_PROGRESS;
  process.env.CLAWCORE_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "clawcore-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.CLAWCORE_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.CLAWCORE_UPDATE_IN_PROGRESS;
  } else {
    process.env.CLAWCORE_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
