...
function resolveMode(cfg: ClawCoreConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("ClawCore doctor");

  const root = await resolveClawCorePackageRoot({
  ...
  let cfg: ClawCoreConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("clawcore configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("clawcore config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("clawcore setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("clawcore doctor --fix")}" to apply changes.`);
  }
