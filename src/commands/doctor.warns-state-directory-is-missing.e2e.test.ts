...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "clawcore-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.CLAWCORE_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when CLAWCORE_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/clawcore.json",
      exists: true,
      raw: "{}",
    ...
    process.env.CLAWCORE_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
