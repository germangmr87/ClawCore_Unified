...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveClawCoreAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("clawcore models auth setup-token")}\` or \`${formatCliCommand("clawcore configure")}\`.`
          : `Run \`${formatCliCommand("clawcore configure")}\` or set an API key env var.`;
