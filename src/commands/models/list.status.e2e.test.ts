...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveClawCoreAgentDir: vi.fn().mockReturnValue("/tmp/clawcore-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/clawcore-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/clawcore-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveClawCoreAgentDir: mocks.resolveClawCoreAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveClawCoreAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/clawcore-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/clawcore-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/clawcore-agent-custom");
