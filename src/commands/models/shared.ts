...
export async function updateConfig(
  mutator: (cfg: ClawCoreConfig) => ClawCoreConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: ClawCoreConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: ClawCoreConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("clawcore agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
