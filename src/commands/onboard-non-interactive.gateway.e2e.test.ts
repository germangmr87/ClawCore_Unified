...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.CLAWCORE_GATEWAY_PASSWORD,
  };
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.CLAWCORE_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "clawcore");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.CLAWCORE_STATE_DIR = stateDir;
    process.env.CLAWCORE_CONFIG_PATH = path.join(stateDir, "clawcore.json");
