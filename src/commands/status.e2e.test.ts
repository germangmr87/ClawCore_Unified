...
beforeAll(() => {
  envSnapshot = captureEnv(["CLAWCORE_PROFILE"]);
  process.env.CLAWCORE_PROFILE = "isolated";
});
...
vi.mock("../infra/clawcore-root.js", () => ({
  resolveClawCorePackageRoot: vi.fn().mockResolvedValue("/tmp/clawcore"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/clawcore/pnpm-lock.yaml",
      markerPath: "/tmp/clawcore/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("ClawCore status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("clawcore status --all") ||
          l.includes("clawcore --profile isolated status --all") ||
          l.includes("clawcore status --all") ||
          l.includes("clawcore --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.CLAWCORE_GATEWAY_TOKEN = "abcd1234";
    try {
