...
const ETC_CLAWCORE_DIR = path.join(ROOT_DIR, "etc", "clawcore");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "clawcore.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcClawCorePath(...parts: string[]) {
  return path.join(ETC_CLAWCORE_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcClawCorePath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "clawcore.json"))).toEqual({
      shared: true,
    });
  });
});
