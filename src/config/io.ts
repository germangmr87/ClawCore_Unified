...
import type { ClawCoreConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareClawCoreVersions } from "./version.js";
...
function coerceConfig(value: unknown): ClawCoreConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ClawCoreConfig;
}
...
function stampConfigVersion(cfg: ClawCoreConfig): ClawCoreConfig {
...
function warnIfConfigFromFuture(cfg: ClawCoreConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareClawCoreVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer ClawCore (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as ClawCoreConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ClawCoreConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as ClawCoreConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: ClawCoreConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as ClawCoreConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.CLAWCORE_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.CLAWCORE_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.CLAWCORE_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.CLAWCORE_WATCH_SESSION === "string" &&
        deps.env.CLAWCORE_WATCH_SESSION.trim().length > 0
          ? deps.env.CLAWCORE_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.CLAWCORE_WATCH_COMMAND === "string" &&
        deps.env.CLAWCORE_WATCH_COMMAND.trim().length > 0
          ? deps.env.CLAWCORE_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `CLAWCORE_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.CLAWCORE_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.CLAWCORE_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): ClawCoreConfig {
