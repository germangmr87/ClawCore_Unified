...
describe("config io write", () => {
  ...
  async function withTempHome(prefix: string, fn: (home: string) => Promise<void>): Promise<void> {
    ...
    await fs.mkdir(path.join(home, ".clawcore"), { recursive: true });
    ...
    process.env.CLAWCORE_STATE_DIR = path.join(home, ".clawcore");
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-config-io-"));
  });
  ...
  it("persists caller changes onto resolved config without leaking runtime defaults", async () => {
    await withTempHome("clawcore-config-io-", async (home) => {
      const configPath = path.join(home, ".clawcore", "clawcore.json");
  ...
  it("appends config write audit JSONL entries with forensic metadata", async () => {
    await withTempHome("clawcore-config-io-", async (home) => {
      const configPath = path.join(home, ".clawcore", "clawcore.json");
      const auditPath = path.join(home, ".clawcore", "logs", "config-audit.jsonl");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        JSON.stringify({ gateway: { port: 18789 } }, null, 2),
  ...
  it("records gateway watch session markers in config audit entries", async () => {
    await withTempHome("clawcore-config-io-", async (home) => {
      ...
      const io = createConfigIO({
        env: {
          CLAWCORE_WATCH_MODE: "1",
          CLAWCORE_WATCH_SESSION: "watch-session-1",
          CLAWCORE_WATCH_COMMAND: "gateway --force",
