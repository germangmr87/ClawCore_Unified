...
import type { ConfigFileSnapshot } from "./types.clawcore.js";
import {
  REDACTED_SENTINEL,
...
function makeSnapshot(config: Record<string, unknown>, raw?: string): ConfigFileSnapshot {
  return {
    path: "/home/user/.clawcore/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/clawcore/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/clawcore/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/clawcore/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/clawcore/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = ClawCoreSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "ClawCoreConfig";
    const hints = mapSensitivePaths(ClawCoreSchema, "", {});
