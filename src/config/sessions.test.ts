...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-clawcore",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-clawcore#general");
  });
  ...
  it("derives session transcripts dir from CLAWCORE_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { CLAWCORE_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.CLAWCORE_STATE_DIR = "/custom/state";
    try {
