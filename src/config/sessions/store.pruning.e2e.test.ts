...
// Keep integration tests deterministic: never read a real clawcore.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.CLAWCORE_SESSION_CACHE_TTL_MS;
    process.env.CLAWCORE_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.CLAWCORE_SESSION_CACHE_TTL_MS;
    } else {
      process.env.CLAWCORE_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
