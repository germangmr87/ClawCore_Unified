...
import type { ClawCoreConfig, ConfigValidationIssue } from "./types.js";
...
import { ClawCoreSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: ClawCoreConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: ClawCoreConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ClawCoreSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ClawCoreConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ClawCoreConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as ClawCoreConfig,
  };
}
