...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses CLAWCORE_HOME for tilde expansion", () => {
    vi.stubEnv("CLAWCORE_HOME", "/srv/clawcore-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/clawcore-home", "cron", "jobs.json"));
  });
});
