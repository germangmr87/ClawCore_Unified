...
describe("agent components", () => {
  const createCfg = (): ClawCoreConfig => ({}) as ClawCoreConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): ClawCoreConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockReset().mockReturnValue("/tmp/clawcore-sessions-test.json");
  });
