...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "clawcore-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    CLAWCORE_GATEWAY_TOKEN: "test-token",
    CLAWCORE_CONFIG_DIR: join(sandbox.rootDir, "config"),
    CLAWCORE_WORKSPACE_DIR: join(sandbox.rootDir, "clawcore"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        CLAWCORE_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        CLAWCORE_EXTRA_MOUNTS: undefined,
        CLAWCORE_HOME_VOLUME: "clawcore-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("CLAWCORE_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("CLAWCORE_EXTRA_MOUNTS=");
    expect(envFile).toContain("CLAWCORE_HOME_VOLUME=clawcore-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("clawcore-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("clawcore-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg CLAWCORE_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
