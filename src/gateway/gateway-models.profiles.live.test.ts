...
import type { ClawCoreConfig, ModelProviderConfig } from "../config/types.js";
import { resolveClawCoreAgentDir } from "../agents/agent-paths.js";
...
import { ensureClawCoreModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.CLAWCORE_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.CLAWCORE_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.CLAWCORE_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<ClawCoreConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.CLAWCORE_AGENT_DIR,
  ...
  const hostAgentDir = resolveClawCoreAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-live-state-"));
  process.env.CLAWCORE_STATE_DIR = tempStateDir;
  ...
  process.env.CLAWCORE_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.clawcore-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveClawCoreAgentDir();
  const sanitizedCfg: ClawCoreConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "ClawCore live tool probe (local, safe): " +
    ...
    process.env.CLAWCORE_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.CLAWCORE_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureClawCoreModelsJson(cfg);
      ...
      const rawModels = process.env.CLAWCORE_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.clawcore-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
