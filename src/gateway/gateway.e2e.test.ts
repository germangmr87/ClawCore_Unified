...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.CLAWCORE_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "clawcore");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.clawcore-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".clawcore");
      ...
      try {
        ...
        process.env.CLAWCORE_CONFIG_PATH = prev.configPath;
        process.env.CLAWCORE_GATEWAY_TOKEN = prev.token;
        process.env.CLAWCORE_SKIP_CHANNELS = prev.skipChannels;
        process.env.CLAWCORE_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.CLAWCORE_SKIP_CRON = prev.skipCron;
        process.env.CLAWCORE_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.CLAWCORE_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.CLAWCORE_STATE_DIR = prev.stateDir;
      process.env.CLAWCORE_CONFIG_PATH = prev.configPath;
      process.env.CLAWCORE_GATEWAY_TOKEN = prev.token;
      process.env.CLAWCORE_SKIP_CHANNELS = prev.skipChannels;
      process.env.CLAWCORE_SKIP_GMAIL_WATCHER = prev.skipGmail;
