...
export function resolveHooksConfig(cfg: ClawCoreConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: ClawCoreConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-clawcore-token"] === "string"
      ? req.headers["x-clawcore-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
