import type { ClawCoreConfig } from "../../config/types.clawcore.js";
...
import { loadClawCorePlugins } from "../../plugins/loader.js";
import {
  ErrorCodes,
  errorShape,
...
function parseValidateConfigFromRawOrRespond(
  ...
  snapshot: Awaited<ReturnType<typeof readConfigFileSnapshot>>,
  respond: RespondFn,
): { config: ClawCoreConfig; schema: ConfigSchemaResponse } | null {
...
function loadSchemaWithPlugins(): ConfigSchemaResponse {
  ...
  const pluginRegistry = loadClawCorePlugins({
    config: cfg,
    cache: true,
    workspaceDir,
    logger: {
  ...
  // our cache. However, both loadConfig() and loadClawCorePlugins() already cache
