...
beforeAll(async () => {
  previousToken = process.env.CLAWCORE_GATEWAY_TOKEN;
  process.env.CLAWCORE_GATEWAY_TOKEN = "test-gateway-token-1234567890";
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.CLAWCORE_GATEWAY_TOKEN = previousToken;
  }
});
...
test.each([
  { clientId: "clawcore-ios", platform: "ios" },
  { clientId: "clawcore-android", platform: "android" },
