...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.CLAWCORE_CONFIG_PATH;
  const prevDisableCache = process.env.CLAWCORE_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "clawcore-plugin-http-auth-test-"));
  const configPath = path.join(dir, "clawcore.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.CLAWCORE_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.CLAWCORE_DISABLE_CONFIG_CACHE;
    } else {
      process.env.CLAWCORE_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
