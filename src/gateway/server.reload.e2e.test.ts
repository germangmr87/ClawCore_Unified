...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.CLAWCORE_SKIP_CHANNELS;
    prevSkipGmail = process.env.CLAWCORE_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.CLAWCORE_SKIP_PROVIDERS;
    process.env.CLAWCORE_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.CLAWCORE_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.CLAWCORE_SKIP_GMAIL_WATCHER;
    } else {
      process.env.CLAWCORE_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.CLAWCORE_SKIP_PROVIDERS;
    } else {
      process.env.CLAWCORE_SKIP_PROVIDERS = prevSkipProviders;
    }
  });
