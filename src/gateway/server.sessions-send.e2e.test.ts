...
const { createClawCoreTools } = await import("../agents/clawcore-tools.js");
...
beforeAll(async () => {
  envSnapshot = captureEnv(["CLAWCORE_GATEWAY_PORT", "CLAWCORE_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.CLAWCORE_GATEWAY_PORT = String(gatewayPort);
  process.env.CLAWCORE_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createClawCoreTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", { timeout: 60_000 }, async () => {
    ...
    if (!configPath) {
      throw new Error("CLAWCORE_CONFIG_PATH missing in gateway test environment");
    }
