...
This directory contains hooks that ship with ClawCore. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.clawcore/clawcore.json`:
...
2. Restart gateway: `pkill -9 -f 'clawcore.*gateway' && pnpm clawcore gateway`
3. Enable the hook: `clawcore hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.clawcore.ai/automation/hooks
