...
homepage: https://docs.clawcore.ai/automation/hooks#command-logger
metadata:
  {
    "clawcore":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.clawcore/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.clawcore/logs/commands.log ~/.clawcore/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/clawcore`:
   ```
   /home/username/.clawcore/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
