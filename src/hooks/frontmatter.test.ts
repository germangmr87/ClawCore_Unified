...
`;
    ...
    expect(parsed.clawcore.emoji).toBe("💾");
    expect(parsed.clawcore.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.clawcore.emoji).toBe("📝");
    expect(parsed.clawcore.events).toEqual(["command"]);
    expect(parsed.clawcore.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.clawcore.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"clawcore": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"clawcore": {"events": ["test"]}}');
  });
...
describe("resolveClawCoreMetadata", () => {
  it("extracts clawcore metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveClawCoreMetadata(frontmatter);
  ...
  it("returns undefined when clawcore key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        clawcore: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with ClawCore" },
            { id: "npm", kind: "npm", package: "@clawcore/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@clawcore/hook");
  });
    ...
    "clawcore":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with ClawCore" }],
      },
  }
---
...
`;
    ...
    const clawcore = resolveClawCoreMetadata(frontmatter);
    expect(clawcore).toBeDefined();
    expect(clawcore?.emoji).toBe("💾");
    expect(clawcore?.events).toEqual(["command:new"]);
    expect(clawcore?.requires?.config).toEqual(["workspace.dir"]);
    expect(clawcore?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const clawcore = resolveClawCoreMetadata(frontmatter);
    expect(clawcore?.emoji).toBe("disk");
    expect(clawcore?.events).toEqual(["command:new"]);
  });
});
