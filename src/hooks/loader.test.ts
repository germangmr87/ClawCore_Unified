...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.CLAWCORE_BUNDLED_HOOKS_DIR;
    process.env.CLAWCORE_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.CLAWCORE_BUNDLED_HOOKS_DIR;
    } else {
      process.env.CLAWCORE_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
