...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__clawcore_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "clawcore-root");
...
describe("resolveClawCorePackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    const { resolveClawCorePackageRootSync } = await import("./clawcore-root.js");

    const project = fx("bin-scenario");
    const argv1 = path.join(project, "node_modules", ".bin", "clawcore");
    const pkgRoot = path.join(project, "node_modules", "clawcore");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "clawcore" }));

    expect(resolveClawCorePackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "clawcore");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "clawcore.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "clawcore" }));

    expect(resolveClawCorePackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveClawCorePackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-clawcore package roots", async () => {
    const { resolveClawCorePackageRootSync } = await import("./clawcore-root.js");

    const pkgRoot = fx("not-clawcore");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-clawcore" }));

    expect(resolveClawCorePackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("async resolver matches sync behavior", async () => {
    const { resolveClawCorePackageRoot } = await import("./clawcore-root.js");

    const pkgRoot = fx("async");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "clawcore" }));

    await expect(resolveClawCorePackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
