...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: ClawCoreConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: ClawCoreConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: ClawCoreConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: ClawCoreConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: ClawCoreConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: ClawCoreConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: ClawCoreConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
