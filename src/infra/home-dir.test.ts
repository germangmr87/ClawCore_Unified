...
describe("resolveEffectiveHomeDir", () => {
  it("prefers CLAWCORE_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/clawcore-home"),
    );
  });
  ...
  it("expands CLAWCORE_HOME when set to ~", () => {
    const env = {
      CLAWCORE_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for CLAWCORE_HOME", () => {
    const result = resolveRequiredHomeDir(
      { CLAWCORE_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when CLAWCORE_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ CLAWCORE_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { CLAWCORE_HOME: "/srv/clawcore-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/clawcore-home")}/x`);
  });
