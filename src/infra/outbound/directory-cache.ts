...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ClawCoreConfig | null = null;
  ...
  get(key: string, cfg: ClawCoreConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: ClawCoreConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: ClawCoreConfig): void {
