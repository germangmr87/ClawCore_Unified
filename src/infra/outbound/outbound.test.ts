...
describe("delivery-queue", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "clawcore-dq-test-"));
  });
...
describe("resolveOutboundSessionRoute", () => {
  const baseConfig = {} as ClawCoreConfig;
  ...
  it("treats Telegram usernames as DMs when unresolved", async () => {
    const cfg = { session: { dmScope: "per-channel-peer" } } as ClawCoreConfig;
    const route = await resolveOutboundSessionRoute({
      cfg,
      channel: "telegram",
      agentId: "main",
