...
let ensureClawCoreCliOnPath: typeof import("./path-env.js").ensureClawCoreCliOnPath;

describe("ensureClawCoreCliOnPath", () => {
  const envKeys = [
    "PATH",
    "CLAWCORE_PATH_BOOTSTRAPPED",
    "CLAWCORE_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureClawCoreCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling clawcore exists", () => {
    const tmp = abs("/tmp/clawcore-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "clawcore");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.CLAWCORE_PATH_BOOTSTRAPPED;

    ensureClawCoreCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.CLAWCORE_PATH_BOOTSTRAPPED = "1";
    ensureClawCoreCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/clawcore-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "clawcore");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/clawcore-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "clawcore");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "clawcore");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/clawcore-path/case-linuxbrew");
