...
function candidateBinDirs(opts: EnsureClawCorePathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `clawcore` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "clawcore");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.CLAWCORE_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "clawcore"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `clawcore` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureClawCoreCliOnPath(opts: EnsureClawCorePathOpts = {}) {
  if (isTruthyEnvValue(process.env.CLAWCORE_PATH_BOOTSTRAPPED)) {
    return;
  }
