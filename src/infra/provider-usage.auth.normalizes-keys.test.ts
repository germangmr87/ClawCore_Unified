...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-provider-auth-suite-"));
  });
  ...
  async function withSuiteHome<T>(
    ...
    await fs.mkdir(path.join(base, ".clawcore", "agents", "main", "sessions"), { recursive: true });
    ...
    process.env.CLAWCORE_STATE_DIR = path.join(base, ".clawcore");
  ...
  it("strips embedded CR/LF from stored auth profiles (token + api_key)", async () => {
    await withSuiteHome(
      async (home) => {
        const agentDir = path.join(home, ".clawcore", "agents", "main", "agent");
