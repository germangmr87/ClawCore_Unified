...
export function triggerClawCoreRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.CLAWCORE_SYSTEMD_UNIT,
        process.env.CLAWCORE_PROFILE,
      );
  ...
  const label =
    process.env.CLAWCORE_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.CLAWCORE_PROFILE);
