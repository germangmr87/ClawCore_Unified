...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.CLAWCORE_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.CLAWCORE_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.CLAWCORE_TEST_TAILSCALE_BINARY;
    } else {
      process.env.CLAWCORE_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
