...
const DEFAULT_PACKAGE_NAME = "clawcore";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "clawcore.mjs");
    ...
    const doctorStep = await runStep(
      step("clawcore doctor", doctorArgv, gitRoot, { CLAWCORE_UPDATE_IN_PROGRESS: "1" }),
    );
