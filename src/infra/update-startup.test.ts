...
describe("update-startup", () => {
  ...
  let resolveClawCorePackageRoot: (typeof import("./clawcore-root.js"))["resolveClawCorePackageRoot"];
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    hadStateDir = Object.prototype.hasOwnProperty.call(process.env, "CLAWCORE_STATE_DIR");
    prevStateDir = process.env.CLAWCORE_STATE_DIR;
    process.env.CLAWCORE_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveClawCorePackageRoot } = await import("./clawcore-root.js"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    vi.mocked(resolveClawCorePackageRoot).mockResolvedValue("/opt/clawcore");
