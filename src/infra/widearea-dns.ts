...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.CLAWCORE_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*clawcore-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "clawcore";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "clawcore");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "clawcore-gw");
  ...
  records.push(`_clawcore-gw._tcp IN PTR ${instanceLabel}._clawcore-gw._tcp`);
  records.push(`${instanceLabel}._clawcore-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._clawcore-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; clawcore-content-hash: ${contentHash}\n${contentBody}`;
}
