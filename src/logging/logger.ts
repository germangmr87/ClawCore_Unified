...
import type { ClawCoreConfig } from "../config/types.js";
import type { ConsoleStyle } from "./console.js";
import { resolvePreferredClawCoreTmpDir } from "../infra/tmp-clawcore-dir.js";
import { readLoggingConfig } from "./config.js";
import { type LogLevel, levelToMinLevel, normalizeLogLevel } from "./levels.js";
import { loggingState } from "./state.js";

export const DEFAULT_LOG_DIR = resolvePreferredClawCoreTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "clawcore.log"); // legacy single-file path

const LOG_PREFIX = "clawcore";
...
function resolveSettings(): ResolvedSettings {
  let cfg: ClawCoreConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => ClawCoreConfig;
      };
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.CLAWCORE_TEST_FILE_LOG !== "1" ? "silent" : "info";
