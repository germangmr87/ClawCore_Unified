...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { CLAWCORE_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { CLAWCORE_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports CLAWCORE_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { CLAWCORE_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { CLAWCORE_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { CLAWCORE_SMOKE: "nope" })).toBe("qr");
  });
});
