...
import { loadClawCorePlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `clawcore-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.CLAWCORE_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "clawcore.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.CLAWCORE_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadClawCorePlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.CLAWCORE_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadClawCorePlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadClawCorePlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
