...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["CLAWCORE_BASE_ENV"]);
    process.env.CLAWCORE_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.CLAWCORE_BASE_ENV ?? "") + "|" + (process.env.CLAWCORE_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { CLAWCORE_TEST_ENV: "ok" },
        },
      );
