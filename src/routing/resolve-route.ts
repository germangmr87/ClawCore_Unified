...
function listAgents(cfg: ClawCoreConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: ClawCoreConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: ClawCoreConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<ClawCoreConfig, EvaluatedBindingsCache>();
