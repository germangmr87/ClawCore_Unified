...
function summarizeGroupPolicy(cfg: ClawCoreConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: ClawCoreConfig): boolean {
...
function collectModels(cfg: ClawCoreConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: ClawCoreConfig): Set<string> {
  const baseCfg: ClawCoreConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: ClawCoreConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: ClawCoreConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: ClawCoreConfig): boolean {
...
function isBrowserEnabled(cfg: ClawCoreConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: ClawCoreConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: ClawCoreConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep CLAWCORE_STATE_DIR on a local-only volume and re-run "${formatCliCommand("clawcore security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: ClawCoreConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer CLAWCORE_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const clawcoreGatewayToken =
    typeof env.CLAWCORE_GATEWAY_TOKEN === "string" && env.CLAWCORE_GATEWAY_TOKEN.trim()
      ? env.CLAWCORE_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-clawcore-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: ClawCoreConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: ClawCoreConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: ClawCoreConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: ClawCoreConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: ClawCoreConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: ClawCoreConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
