...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: ClawCoreConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawcore-security-audit-"));
  });
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.CLAWCORE_GATEWAY_TOKEN;
    const prevPassword = process.env.CLAWCORE_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.CLAWCORE_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.CLAWCORE_GATEWAY_PASSWORD;
      } else {
        process.env.CLAWCORE_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    ...
    const configPath = path.join(stateDir, "clawcore.json");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: ClawCoreConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.CLAWCORE_STATE_DIR;
    const tmp = await makeTmpDir("discord");
    process.env.CLAWCORE_STATE_DIR = tmp;
    ...
    try {
      ...
      if (prevStateDir == null) {
        ...
        process.env.CLAWCORE_STATE_DIR = prevStateDir;
      }
    }
  });
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: ClawCoreConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.CLAWCORE_GATEWAY_TOKEN;
    process.env.CLAWCORE_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: ClawCoreConfig = {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.clawcore",
      configPath: "/Users/test/Dropbox/.clawcore/clawcore.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: ClawCoreConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        clawcore: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        clawcore: { extensions: ["../outside.js"] },
      }),
    );
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.CLAWCORE_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.CLAWCORE_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.CLAWCORE_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.CLAWCORE_GATEWAY_PASSWORD;
      } else {
        process.env.CLAWCORE_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env password over remote password", async () => {
      process.env.CLAWCORE_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: ClawCoreConfig = {
        gateway: {
          mode: "remote",
