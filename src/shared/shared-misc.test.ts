...
describe("shared/frontmatter", () => {
  ...
  test("resolveClawCoreManifestBlock parses JSON5 metadata and picks clawcore block", () => {
    const frontmatter = {
      metadata: "{ clawcore: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveClawCoreManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveClawCoreManifestBlock returns undefined for invalid input", () => {
    expect(resolveClawCoreManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveClawCoreManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveClawCoreManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
