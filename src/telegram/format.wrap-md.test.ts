...
describe("edge cases", () => {
  ...
  it("keeps .co domains as links", () => {
    const result = markdownToTelegramHtml("Visit t.co and clawcore.co");
    expect(result).toContain('<a href="http://t.co">');
    expect(result).toContain('<a href="http://clawcore.co">');
    expect(result).not.toContain("<code>t.co</code>");
    expect(result).not.toContain("<code>clawcore.co</code>");
  });
