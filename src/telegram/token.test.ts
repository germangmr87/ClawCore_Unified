...
function withTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "clawcore-telegram-token-"));
}

async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.CLAWCORE_STATE_DIR;
  const dir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "clawcore-telegram-"));
...
describe("resolveTelegramToken", () => {
  ...
  it("uses tokenFile when configured", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile } } } as ClawCoreConfig;
    const res = resolveTelegramToken(cfg);
