...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "clawcore-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "clawcore-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "clawcore-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "clawcore-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.clawcore when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "clawcore-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers CLAWCORE_HOME over HOME", () => {
    vi.stubEnv("CLAWCORE_HOME", "/srv/clawcore-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/clawcore-home"));
...
describe("shortenHomePath", () => {
  it("uses $CLAWCORE_HOME prefix when CLAWCORE_HOME is set", () => {
    vi.stubEnv("CLAWCORE_HOME", "/srv/clawcore-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/clawcore-home")}/.clawcore/clawcore.json`)).toBe(
      "$CLAWCORE_HOME/.clawcore/clawcore.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $CLAWCORE_HOME replacement when CLAWCORE_HOME is set", () => {
    vi.stubEnv("CLAWCORE_HOME", "/srv/clawcore-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/clawcore-home")}/.clawcore/clawcore.json`),
    ).toBe("config: $CLAWCORE_HOME/.clawcore/clawcore.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/clawcore")).toBe(path.resolve(os.homedir(), "clawcore"));
  });
  ...
  it("prefers CLAWCORE_HOME for tilde expansion", () => {
    vi.stubEnv("CLAWCORE_HOME", "/srv/clawcore-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/clawcore")).toBe(path.resolve("/srv/clawcore-home", "clawcore"));
