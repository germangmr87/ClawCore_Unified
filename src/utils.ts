...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".clawcore");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.CLAWCORE_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$CLAWCORE_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via CLAWCORE_STATE_DIR.
