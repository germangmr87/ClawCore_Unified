...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "clawcore",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/clawcore.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("clawcore");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "clawcore");
