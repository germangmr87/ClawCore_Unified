...
process.env.CLAWCORE_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
// Set HOME/state isolation before importing any runtime ClawCore modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: ClawCoreConfig) => {
    ...
    resolveAccount: (cfg: ClawCoreConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: ClawCoreConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
