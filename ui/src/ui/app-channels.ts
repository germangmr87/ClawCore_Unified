...
export async function handleWhatsAppStart(host: ClawCoreApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: ClawCoreApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: ClawCoreApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: ClawCoreApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: ClawCoreApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: ClawCoreApp): string {
...
function resolveGatewayHttpAuthHeader(host: ClawCoreApp): string | null {
...
function buildGatewayHttpHeaders(host: ClawCoreApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: ClawCoreApp) {
...
export function handleNostrProfileToggleAdvanced(host: ClawCoreApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: ClawCoreApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: ClawCoreApp) {
  const state = host.nostrProfileFormState;
