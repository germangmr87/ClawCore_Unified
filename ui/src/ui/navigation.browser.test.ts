...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/clawcore/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/clawcore");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/clawcore/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__CLAWCORE_CONTROL_UI_BASE_PATH__ = "/clawcore";
    const app = mountApp("/clawcore/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/clawcore");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/clawcore/sessions");
  });
